...
export function App({ initialActiveTab = 'hotel' }: AppProps) {
...
Téléphone: ${formData.phone}`;
  ...
  const handleCRMSubmit = async () => {
    try {
      // Créer Contact dans Salesforce
      const contactRes = await fetch('/api/createSalesforceContact', {
        method: 'POST',
      ...
      // Préparer le payload pour l'opportunité Salesforce
      ...
      // Créer Opportunité dans Salesforce
      const oppRes = await fetch('/api/createSalesforceOpportunity', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!oppRes.ok) throw new Error(`Erreur création opportunité: ${oppData.detail}`);
      
      // Succès
      console.log("Opportunité créée avec succès:", oppData.opportunityId);
    } catch (error) {
